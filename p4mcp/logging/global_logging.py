...
class GlobalLogger:
    ...
    def setup(self, log_level: str = LoggingConfig.DEFAULT_LOG_LEVEL,
        ...
        self._setup_global_handlers(root_logger, log_file, log_dir)
        self._suppress_noisy_loggers()
        self._setup_complete = True
    
    def _setup_global_handlers(self, root_logger: logging.Logger, log_file: Optional[str], log_dir: Optional[str]) -> None:
        """Setup global logging handlers"""
        ...
        # Priority: log_file (full path) > log_dir (directory only) > default
...
def setup_logging(log_level: str = LoggingConfig.DEFAULT_LOG_LEVEL,
                  ...
                  disable_logging: bool = False) -> None:
    """Setup global logging configuration"""
    _global_logger.setup(log_level, log_file, log_dir, disable_logging)
