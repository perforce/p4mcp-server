...
class QueryFilesParams(PaginatedParams):
    ...
    action: FileAction = Field(
        ...
        examples=["content", "history", "info", "metadata", "diff", "annotations", "search", "grep"]
    )
    file_path: str = Field(
        min_length=1,
    ...
    pattern: Optional[str] = Field(
        ...
        examples=["*.py", "TODO", "class.*Handler"]
    )
    ...
    @model_validator(mode='after')
    def validate_search_grep_params(self):
        """Validate search and grep-specific parameters."""
        if self.action in (FileAction.SEARCH, FileAction.GREP) and not self.pattern:
            raise ValueError('pattern is required for search and grep actions')
        return self
