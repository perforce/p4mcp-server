...
class WorkspacesHandlers:
    ...
    async def _handle_modify_workspaces(self, params):
        if not params.workspace_name and params.action in ["delete", "update", "switch"]:
            logger.error(f"workspace_name is required for this {params.action} action")
            raise ValueError(f"workspace_name is required for this {params.action} action")
        ...
        if params.action == "create":
            ...
            result = await self.workspace_services.delete_workspace(params.workspace_name)
        elif params.action == "update":
            result = await self.workspace_services.update_workspace(params.workspace_name, {k: v for k, v in params.specs.model_dump().items() if v is not None})
        elif params.action == "switch":
            result = await self.workspace_services.switch_workspace(params.workspace_name)
        else:
