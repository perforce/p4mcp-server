...
then upload to the Logstash endpoint as NDJSON.
...
def send_to_logstash(endpoint, docs):
    """Send documents to Logstash as NDJSON."""
    if not docs:
    ...
    if body.lower() in ("ok", "success", "accepted"):
        logger.info(f"Upload successful: {body}")
        return True
    ...
    try:
        ...
        logger.warning(f"Non-JSON response: {body[:200]}")
...
def collect_traffic_views(owner, repo, headers):
    ...
    for bucket in data.get("views", []):
        ts = int(datetime.fromisoformat(bucket["timestamp"].replace("Z", "+00:00")).timestamp() * 1000)
...
def collect_release_downloads(owner, repo, headers):
    """Fetch download counts for release assets."""
...
def main():
    ...
    try:
        ...
        logger.info(f"Collected stars for {full_repo}")
    ...
    try:
        ...
        logger.info(f"Collected {len(views_docs)} traffic views records")
    except requests.exceptions.RequestException as e:
    ...
    try:
        ...
        logger.info(f"Collected {len(clones_docs)} traffic clones records")
    except requests.exceptions.RequestException as e:
    ...
    try:
        ...
        logger.info(
            f"Collected {len(release_docs)} release records "
